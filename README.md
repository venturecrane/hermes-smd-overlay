...
| `hermes-smd-webhook-router` | `pre_gateway_dispatch` | Routes inbound webhook payloads to skills via `customer.yaml.webhook_triggers[]`. Emits `WEBHOOK_ROUTED` + attaches an inbound provenance envelope and emits `INBOUND_RECEIVED` (ADR 0021 Stream E + ADR 0027). On a verified sender who is on the authored roster, it also inserts the SENDER STATUS work-request paragraph into the dispatched email prompt, above the untrusted-body delimiter (ss-console#2416) — the only seam that can change the primary user message of a webhook turn. |
