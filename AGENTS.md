...
- **customer.yaml authoring + validation** lives in `ss-console/src/lib/ai-employee/customer-yaml/`. The `bootstrap/translate.py` here re-implements the materialization to per-profile config.yaml, not the validation.
- **Skill catalog** lives in `ss-console/ai-employee/skills/`. Published into Machine images at build time; not part of this overlay.
