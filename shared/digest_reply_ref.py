...
def append_number_ok(append: dict) -> bool:
    """An envelope append's ``n`` is well-formed: absent or null (an envelope
    written before numbering, or a row the body printed no number for) or a
    valid digest number. Anything else is malformed, and a malformed append
    refuses the whole envelope, as every other malformed field does."""
    number = append.get("n")
    return number is None or valid_digest_number(number)
...
def stamp_raise(event: dict, append: dict, dispatch_ref: str) -> dict:
    ...
    Only raises carry them. ``n`` is copied only when the envelope authored a
    valid one; ``thread_ref`` is never set here (the broker stamps it)."""
